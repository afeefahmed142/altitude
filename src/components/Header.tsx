        ...
        {currentTab === 'forms' ? (
            ...
            {(['build', 'settings', 'logic'] as const).map((tab) => {
              ...
              return (
                ...
                >
                  {tab}
                  {isActive && (
                    <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#3b2bee] rounded-full" />
                  )}
                </button>
              );
            })}
          ...
          <div className="relative w-80">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white border border-[#e5e7eb] rounded-xl text-xs text-[#1f2937] placeholder-[#9ca3af] focus:outline-none focus:border-[#3b2bee] focus:ring-1 focus:ring-[#3b2bee] transition-all"
            />
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-3.5">
        ...
        {currentTab === 'forms' && (
          <div
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium text-[#4b5563]"
          >
        ...
        {currentTab === 'forms' && (
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-2 text-xs font-medium text-[#374151] hover:text-[#111827] bg-[#f9fafb] hover:bg-[#f3f4f6] border border-[#e5e7eb] rounded-xl transition-all cursor-pointer"
          >
        ...
        <button
          id="btn-publish-form"
          onClick={onPublish}
          className="flex items-center gap-2 px-4 py-2 text-xs font-medium text-white bg-[#3b2bee] hover:bg-[#3020d6] active:scale-95 rounded-xl shadow-sm hover:shadow transition-all cursor-pointer"
        >
