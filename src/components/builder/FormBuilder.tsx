...
export const FormBuilder: React.FC<FormBuilderProps> = ({
  ...
  return (
    <div className="flex-1 flex overflow-hidden bg-[#fbfbfe]">
      {/* Left Elements Palette Sidebar matching Image 1 */}
      <aside
        id="builder-palette"
        className="w-68 bg-white border-r border-[#e5e7eb] p-4 flex flex-col gap-5 overflow-y-auto shrink-0 select-none"
      >
        {/* Search Elements */}
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8.5 pr-3 py-2 bg-white border border-[#e5e7eb] rounded-lg text-xs text-[#1f2937] placeholder-[#9ca3af] focus:outline-none focus:border-[#3b2bee]"
          />
        </div>
            ...
            {filteredBasic.map((item) => {
              ...
              return (
                <button
                  ...
                  className="flex flex-col items-center justify-center p-3 rounded-lg border border-[#e5e7eb] bg-white hover:bg-[#f8f9ff] hover:border-[#c7d2fe] transition-all cursor-pointer group text-center gap-1.5 shadow-2xs hover:shadow-xs active:scale-[0.98]"
                >
                  <Icon className="w-4 h-4 text-[#4b5563] group-hover:text-[#3b2bee] transition-colors" />
            ...
            {filteredAdvanced.map((item) => {
              ...
              return (
                <button
                  ...
                  id={`btn-add-${item.type}`}
                  onClick={() => handleAddField(item.type)}
                  className="flex items-center gap-3 p-2.5 rounded-lg border border-[#e5e7eb] bg-white hover:bg-[#f8f9ff] hover:border-[#c7d2fe] transition-all cursor-pointer group shadow-2xs hover:shadow-xs text-left"
                >
                  <div className="w-7 h-7 rounded-md bg-[#eef2ff] text-[#3b2bee] flex items-center justify-center shrink-0">
        ...
        <div className="pt-2 border-t border-[#f3f4f6]">
          <button
            ...
            className="w-full py-2.5 px-3 bg-[#eef2ff] hover:bg-[#e0e7ff] text-[#3b2bee] text-xs font-bold rounded-xl flex items-center justify-center gap-2 transition-colors cursor-pointer"
          >
      ...
      >
        <div className="max-w-2xl mx-auto space-y-4 pb-24">
          ...
          >
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full text-2xl font-black text-[#111827] placeholder-[#9ca3af] focus:outline-none border-b border-transparent focus:border-[#3b2bee] pb-1 transition-all"
              />
            </div>
            <div className="space-y-1 mt-3">
              ...
              <input
                ...
                className="w-full text-xs text-[#6b7280] placeholder-[#9ca3af] focus:outline-none border-b border-transparent focus:border-[#3b2bee] pt-1 pb-1 transition-all"
              />
            </div>
          </div>
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl border p-5 shadow-2xs transition-all relative group ${
                  isActive
                    ? 'border-[#3b2bee] ring-2 ring-[#3b2bee]/20 shadow-sm'
              ...
              >
                ...
                <div className="flex items-center justify-between gap-3 mb-3">
                  <div className="flex items-center gap-2 flex-1">
                    ...
                    <div className="flex-1 relative flex items-center">
                      <input
                        ...
                        className="w-full font-bold text-sm text-[#111827] placeholder-[#9ca3af] px-2 py-1 rounded-lg border border-transparent hover:border-[#e5e7eb] focus:border-[#3b2bee] focus:bg-white focus:outline-none transition-all"
                      />
                      {field.required && (
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <select
                      ...
                      className="text-[11px] font-semibold bg-[#f9fafb] border border-[#e5e7eb] rounded-lg px-2 py-1 text-[#374151] focus:outline-none focus:border-[#3b2bee] cursor-pointer"
                    >
                ...
                {isActive && (
                  <div className="mb-3 px-2">
                    <input
                      ...
                      className="w-full text-xs text-[#6b7280] placeholder-[#9ca3af] bg-transparent border-b border-[#f3f4f6] focus:border-[#3b2bee] pb-1 focus:outline-none transition-all"
                    />
                  </div>
                )}
                ...
                {field.type === 'multiple_choice' && (
                  <div className="mt-2 px-2 space-y-2">
                      ...
                      (opt, optIdx) => (
                        ...
                        >
                          ...
                          <input
                            ...
                            className="flex-1 px-2 py-1 rounded border border-transparent hover:border-[#e5e7eb] focus:border-[#3b2bee] focus:outline-none text-xs text-[#111827]"
                          />
                    ...
                    <button
                      ...
                      className="text-xs font-semibold text-[#3b2bee] hover:text-[#3020d6] flex items-center gap-1 mt-2 cursor-pointer"
                    >
                ...
                {field.type === 'digital_signature' && (
                  <div className="mt-2 px-2">
                    <div className="border border-dashed border-[#c7d2fe] bg-[#f8f9ff] rounded-xl p-4 text-center text-xs text-[#3b2bee] flex items-center justify-center gap-2">
                ...
                {field.type === 'calendar_booking' && (
                  <div className="mt-2 px-2">
                    <div className="border border-dashed border-[#e5e7eb] rounded-xl p-3 text-xs text-[#6b7280] flex items-center gap-2">
                      <Calendar className="w-4 h-4 text-[#3b2bee]" />
                ...
                {field.type === 'voice_input' && (
                  <div className="mt-2 px-2">
                    <div className="border border-dashed border-[#e5e7eb] rounded-xl p-3 text-xs text-[#6b7280] flex items-center gap-2">
                      <Mic className="w-4 h-4 text-[#3b2bee]" />
                ...
                {field.type === 'multi_language' && (
                  <div className="mt-2 px-2">
                    <div className="border border-dashed border-[#e5e7eb] rounded-xl p-3 text-xs text-[#6b7280] flex items-center gap-2">
                      <Languages className="w-4 h-4 text-[#3b2bee]" />
          ...
          {/* Card 3: AI Suggested Block matching Image 1 */}
          {showAiSuggestedCard && (
            <div
              id="ai-suggested-card"
              className="bg-white rounded-2xl border border-[#c7d2fe] p-5 shadow-sm relative transition-all ring-1 ring-[#e0e7ff]"
            >
              {/* Floating Sparkle icon */}
              <div className="absolute -left-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-[#f5f3ff] border border-[#ddd6fe] text-[#7c3aed] flex items-center justify-center shadow-xs">
                <Sparkles className="w-3 h-3" />
              </div>

              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-sm font-bold text-[#3b2bee]">
                    Appointment Preference
                  </h3>
                  <div className="inline-block mt-1 px-2 py-0.5 rounded-full bg-[#eef2ff] text-[#3b2bee] text-[9px] font-bold tracking-wider uppercase font-mono">
                    AI SUGGESTED
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    id="btn-dismiss-ai-suggested"
                    onClick={() => setShowAiSuggestedCard(false)}
                    className="text-[#9ca3af] hover:text-[#4b5563] p-1 rounded-md hover:bg-[#f3f4f6] cursor-pointer"
                  >
                    <X className="w-4 h-4" />
                  </button>
                  <button
                    id="btn-add-ai-suggested"
                    onClick={handleAddAiSuggestedAppointment}
                    className="bg-[#3b2bee] hover:bg-[#3020d6] text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-2xs transition-all cursor-pointer flex items-center gap-1"
                  >
              ...
              <div className="mt-4 space-y-2 pl-1">
                <div className="flex items-center gap-2.5 text-xs text-[#374151]">
                  <div className="w-3.5 h-3.5 rounded-full border border-[#9ca3af]" />
          ...
          <div
            id="dropzone-area"
            onClick={() => handleAddField('short_text')}
            className="border-2 border-dashed border-[#d1d5db] hover:border-[#3b2bee] rounded-2xl p-8 text-center bg-[#fafafc] hover:bg-[#f5f6ff] transition-all cursor-pointer group flex flex-col items-center justify-center gap-2"
          >
            <div className="w-9 h-9 rounded-full bg-white border border-[#e5e7eb] group-hover:border-[#3b2bee] group-hover:text-[#3b2bee] text-[#6b7280] flex items-center justify-center shadow-2xs transition-all">
              <Plus className="w-4 h-4" />
            </div>
            <span className="text-xs font-bold text-[#6b7280] group-hover:text-[#3b2bee] transition-colors">
      ...
      {/* Right AI Sidekick matching Image 1 */}
      <AISidekick
        onAddFields={handleAddFieldsFromAi}
