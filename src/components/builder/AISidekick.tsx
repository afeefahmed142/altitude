...
export const AISidekick: React.FC<AISidekickProps> = ({
  ...
  return (
    <aside
      ...
      className="w-84 bg-white border-l border-[#e5e7eb] flex flex-col h-[calc(100vh-4rem)] shrink-0 select-none"
    >
      {/* Header matching Image 1 */}
      <div className="px-5 py-4 border-b border-[#f3f4f6] flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-6 h-6 rounded-md bg-[#eef2ff] text-[#3b2bee] flex items-center justify-center">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg) => (
          ...
          >
            {msg.sender === 'ai' && (
              <div className="w-7 h-7 rounded-full bg-[#f5f3ff] text-[#7c3aed] flex items-center justify-center shrink-0 border border-[#ddd6fe]">
            ...
            <div
              className={`max-w-[85%] rounded-2xl p-3.5 text-xs leading-relaxed ${
                msg.sender === 'user'
                  ? 'bg-[#3b2bee] text-white rounded-tr-none shadow-sm'
                  : 'bg-[#f9fafb] text-[#1f2937] border border-[#e5e7eb] rounded-tl-none'
              }`}
            >
              <p>{msg.text}</p>
              {msg.appliedAction && (
                <div className="mt-2 pt-2 border-t border-[#e5e7eb] flex items-center gap-1.5 text-[11px] text-[#4f46e5] font-medium">
        ...
        {isLoading && (
          <div className="flex items-start gap-2.5">
            ...
            <div className="bg-[#f9fafb] border border-[#e5e7eb] rounded-2xl rounded-tl-none p-3.5 text-xs text-[#6b7280] flex items-center gap-2">
              <Loader2 className="w-3.5 h-3.5 animate-spin text-[#3b2bee]" />
      ...
      <div className="p-4 border-t border-[#f3f4f6] bg-white">
        <div className="border border-[#e5e7eb] rounded-2xl p-2.5 focus-within:border-[#3b2bee] focus-within:ring-1 focus-within:ring-[#3b2bee] transition-all bg-[#fafafa]">
          <textarea
          ...
          <div className="flex items-center justify-between pt-1 border-t border-[#f0f1f4] mt-1">
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors cursor-pointer ${
                isListening
                  ...
                  : 'text-[#9ca3af] hover:text-[#4b5563] hover:bg-[#eef2ff]'
              }`}
            >
              {isListening ? (
            ...
            <button
              ...
              className="w-7 h-7 bg-[#3b2bee] hover:bg-[#3020d6] disabled:opacity-40 text-white rounded-lg flex items-center justify-center transition-all cursor-pointer disabled:cursor-not-allowed shadow-xs"
            >
