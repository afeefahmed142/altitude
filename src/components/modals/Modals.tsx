    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-lg w-full border border-[#e5e7eb] shadow-xl overflow-hidden animate-in fade-in zoom-in-95">
        ...
        <div className="p-6 space-y-4">
          <div className="p-3 bg-[#f5f3ff] border border-[#ddd6fe] rounded-xl text-xs text-[#6d28d9]">
      ...
      <div className="bg-white rounded-2xl max-w-xl w-full border border-[#e5e7eb] shadow-xl overflow-hidden animate-in fade-in zoom-in-95">
        ...
        <div className="p-6 space-y-4">
          <p className="text-xs text-[#4b5563] leading-relaxed">
            FormX Bot Radar identified <strong>6 suspicious submissions</strong> exhibiting automated robotic signatures (sub-second completion times & burst requests).
          </p>
          ...
          <div className="flex border-b border-[#e5e7eb] gap-4">
            <button
              ...
              className={`pb-2.5 text-xs font-semibold cursor-pointer ${
                activeShareTab === 'link' ? 'text-[#3b2bee] border-b-2 border-[#3b2bee]' : 'text-[#6b7280]'
              }`}
            >
              Direct Link
            </button>
            <button
              ...
              className={`pb-2.5 text-xs font-semibold cursor-pointer ${
                activeShareTab === 'embed' ? 'text-[#3b2bee] border-b-2 border-[#3b2bee]' : 'text-[#6b7280]'
              }`}
            >
              Embed HTML
            </button>
            <button
              ...
              className={`pb-2.5 text-xs font-semibold cursor-pointer ${
                activeShareTab === 'qr' ? 'text-[#3b2bee] border-b-2 border-[#3b2bee]' : 'text-[#6b7280]'
              }`}
            >
              QR Code
            </button>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-3.5 py-2 bg-[#3b2bee] text-white rounded-xl text-xs font-bold flex items-center gap-1 cursor-pointer shrink-0"
                >
          ...
          {activeShareTab === 'embed' && (
            <div className="space-y-2">
              ...
              <button
                ...
                className="w-full py-2 bg-[#3b2bee] text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1 cursor-pointer"
              >
                {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
          ...
          {activeShareTab === 'qr' && (
            <div className="flex flex-col items-center justify-center p-4 space-y-3">
              <div className="w-36 h-36 border-2 border-[#e5e7eb] rounded-2xl p-2 bg-white flex items-center justify-center shadow-xs">
                <QrCode className="w-32 h-32 text-[#3b2bee]" />
              </div>
...
export const NewFolderModal: React.FC<NewFolderModalProps> = ({
  ...
  const [color, setColor] = useState('#3b2bee');
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-md w-full border border-[#e5e7eb] shadow-xl overflow-hidden animate-in fade-in zoom-in-95">
        <div className="px-6 py-4 border-b border-[#f3f4f6] flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm font-bold text-[#111827]">
            <FolderPlus className="w-4 h-4 text-[#3b2bee]" />
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full px-3.5 py-2 border border-[#d1d5db] rounded-xl text-xs text-[#111827] focus:outline-none focus:border-[#3b2bee]"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-3">
              {['#3b2bee', '#7c3aed', '#059669', '#d97706', '#dc2626', '#475569'].map((c) => (
                <button
                  key={c}
                  type="button"
                  onClick={() => setColor(c)}
                  className={`w-7 h-7 rounded-full transition-transform cursor-pointer ${
                    color === c ? 'ring-2 ring-offset-2 ring-[#3b2bee] scale-110' : 'hover:scale-105'
                  }`}
          ...
          <div className="pt-4 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              className="px-5 py-2 text-xs font-bold text-white bg-[#3b2bee] hover:bg-[#3020d6] rounded-xl shadow-xs transition-all cursor-pointer"
            >
              Create Folder
            </button>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <form onSubmit={handleInvite} className="space-y-3 bg-[#fafafa] p-4 border border-[#e5e7eb] rounded-xl">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                type="submit"
                className="px-4 py-1.5 bg-[#3b2bee] hover:bg-[#3020d6] text-white text-xs font-bold rounded-lg transition-all cursor-pointer"
              >
                Send Invite
              </button>
            </div>
